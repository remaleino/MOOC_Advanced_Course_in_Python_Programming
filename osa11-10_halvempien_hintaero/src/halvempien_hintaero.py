class Asunto:
    ...
    def hintaero(self, verrattava):
        ...
        ero = abs((self.neliohinta * self.nelioita) -
                  (verrattava.neliohinta * verrattava.nelioita))
        return ero

    def kalliimpi(self, verrattava):
        ero = (self.neliohinta * self.nelioita) - \
            (verrattava.neliohinta * verrattava.nelioita)
        return ero > 0
...
def halvemmat(asunnot: list, verrattava: Asunto):
    return [(a, a.hintaero(verrattava)) for a in asunnot if verrattava.kalliimpi(a) == True]
