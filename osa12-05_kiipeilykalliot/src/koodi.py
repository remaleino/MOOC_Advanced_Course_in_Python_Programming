...
def reittien_maaran_mukaan(lista: list):
    def maara(a: Kiipeilykallio):
        return a.reitteja()
    return sorted(lista, key=maara)


def vaikeimman_reitin_mukaan(lista: list):
    def maara(a: Kiipeilykallio):
        return a.vaikein_reitti().grade
    return sorted(lista, key=maara, reverse=True)
